...
const genreImages = {
  'trap': 'https://images.unsplash.com/photo-1505964253539-4ca5a36328dd?q=80&w=3254&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
  'hip-hop': 'https://images.unsplash.com/photo-1623531249239-07774b804ac8?q=80&w=1012&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
  'drill': 'https://images.unsplash.com/photo-1629753863735-4c9ba15bc10b?q=80&w=987&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
  'rnb': 'https://images.unsplash.com/photo-1571463839048-120efd670f1e?q=80&w=2070&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
  'pop': 'https://images.unsplash.com/photo-1470225620780-dba8ba36b745?q=80&w=2940&auto=format&fit=crop'
};
...
export default function GenrePage() {
  ...
  const genreTitle = useMemo(() => genre ? genre.charAt(0).toUpperCase() + genre.slice(1) : "Beats", [genre]);
  const genreBanner = useMemo(() => genreImages[genre] || null, [genre]);
  
  // SEO optimized titles and descriptions
  const seoTitle = useMemo(() => genre ? `Royalty Free ${genre.charAt(0).toUpperCase() + genre.slice(1)} Beats` : "Royalty Free Beats", [genre]);
  const seoSubtitle = useMemo(() => {
    const year = new Date().getFullYear();
    return genre ? `Free ${genre} beats for commercial use ${year} - Download high-quality instrumentals` : "Free beats for commercial use";
  }, [genre]);
  ...
  // Effect to update document title based on genre state
  useEffect(() => {
    if (genre) {
      document.title = `${genre.charAt(0).toUpperCase() + genre.slice(1)} Beats | RoyaltyFreeBeats.io`;
    } else {
      document.title = "Royalty Free Beats | RoyaltyFreeBeats.io"; // Default title if no genre is set
    }
  }, [genre]);
  
  useEffect(() => { if (genre) loadBeats(genre); }, [genre]);
  useEffect(() => { filterBeats(); }, [beats, searchTerm]);

  const loadBeats = async (genreToLoad) => {
    setLoading(true);
    try {
      const data = await Beat.filter({ genre: genreToLoad }, '-created_date');
      setBeats(data);
    ...
    <div className="bg-[var(--background)] pb-20">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative text-center py-16 sm:py-24 overflow-hidden rounded-2xl mt-8">
          {genreBanner && (
            <>
              <div className="absolute inset-0 bg-cover bg-center bg-no-repeat rounded-2xl" style={{ backgroundImage: `url(${genreBanner})` }} />
        ...
        <div className="text-center mt-12 mb-8 px-4">
          <h3 className="text-xl font-semibold text-[var(--text-primary)] mb-3">
            Premium {genreTitle} Instrumentals for Artists & Content Creators
          </h3>
        ...
        {filteredBeats.length === 0 ? (
          <div className="text-center py-20">
            <p className="text-[var(--text-secondary)]">No {genreTitle} beats found matching your search.</p>
          </div>
        ) : (
        ...
        <div className="mt-16 bg-white rounded-xl p-8 border border-[var(--border)]">
          <h4 className="text-lg font-semibold text-[var(--text-primary)] mb-4">
            Why Choose Our {genreTitle} Beats?
          </h4>
