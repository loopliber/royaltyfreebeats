...
const GENRE = 'boom-bap';
const GENRE_TITLE = 'Boom Bap';
const GENRE_BANNER = 'https://images.unsplash.com/photo-1493225457124-a3eb161ffa5f?q=80&w=2070&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D';

export default function BoomBapPage() {
  ...
  // Memoize loadBeats to prevent unnecessary re-renders and satisfy useEffect dependencies
  const loadBeats = useCallback(async () => {
    ...
    setLoading(false);
  }, []); // Dependencies: setLoading and setBeats are stable setters, Beat.filter is stable.

  // Memoize filterBeats to prevent unnecessary re-renders and satisfy useEffect dependencies
  const filterBeats = useCallback(() => {
  ...
  }, [beats, searchTerm, setFilteredBeats]); // Dependencies: beats, searchTerm, and setFilteredBeats (stable setter)

  useEffect(() => {
    document.title = `${GENRE_TITLE} Beats | RoyaltyFreeBeats.io`;
    loadBeats();
  }, [loadBeats]); // loadBeats is now a stable function

  useEffect(() => { filterBeats(); }, [beats, searchTerm, filterBeats]); // filterBeats is now a stable function
  ...
  }, [currentBeat, filteredBeats, setCurrentBeat, setCurrentBeatIndex]);
  ...
  }, [currentBeatIndex, filteredBeats, setCurrentBeatIndex, setCurrentBeat]);
  ...
  const handleBeatEnd = useCallback(() => handleNext(), [handleNext]);

  const handleDownloadClick = useCallback((beat) => {
    setBeatForDownload(beat);
    setIsYouTubeModalOpen(true);
  }, [setBeatForDownload, setIsYouTubeModalOpen]);

  const handlePurchaseClick = useCallback((beat) => {
    setBeatForPurchase(beat);
    setIsLicenseModalOpen(true);
  }, [setBeatForPurchase, setIsLicenseModalOpen]);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mt-12 mb-8 px-4">
          ...
          <p className="text-[var(--text-secondary)] max-w-4xl mx-auto leading-relaxed">
            ...
            Classic 90s hip hop sound with vinyl samples and jazz loops. All instrumentals come with professional mixing and are cleared for YouTube, Spotify, and other platforms. 
        ...
        <div className="mt-16 bg-white rounded-xl p-8 border border-[var(--border)]">
          ...
          <div className="grid md:grid-cols-2 gap-6 text-sm text-[var(--text-secondary)] leading-relaxed">
            <div>
              <h5 className="font-medium text-[var(--text-primary)] mb-2">Authentic 90s Sound</h5>
              <p>Classic boom bap beats with jazz samples, vinyl crackle, and hard-hitting drums that capture the golden era of hip hop.</p>
            </div>
            <div>
